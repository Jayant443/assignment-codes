...
class AStarSearch:
    def __init__(self, start_state, goal_state):
        ...
        self.goal_pos = {1: (0, 0), 2: (0, 1), 3: (0, 2), 4: (1, 0), 5: (1, 1), 6: (1, 2), 7: (2, 0), 8: (2, 1)}
    ...
    def calculate_heuristic(self, state):
        ...
        return distance
    def is_goal(self, state): return state == self.goal_state
    def state_to_tuple(self, state): return tuple(tuple(row) for row in state)
    ...
    def solve(self):
        ...
        while open_list:
            ...
            if self.is_goal(current.state): return self.reconstruct_path(current)
